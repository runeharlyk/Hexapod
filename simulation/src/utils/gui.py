...
class GUI:
    def __init__(self, bot):
        ...
        self.velocity_mm_s_slider = p.addUserDebugParameter("Velocity (mm/s)", 0, 200, 0)
        self.direction_rad_slider = p.addUserDebugParameter("Direction (rad)", 0, 2 * np.pi, np.pi)
        self.turn_rate_rad_s_slider = p.addUserDebugParameter("Turn rate (rad/s)", -np.pi / 4, np.pi / 4, 0)
    ...
    def update_gait_state(self, gait_state: GaitStateT):
        gait_state["velocity_mm_s"] = p.readUserDebugParameter(self.velocity_mm_s_slider)
        gait_state["direction_rad"] = p.readUserDebugParameter(self.direction_rad_slider)
        gait_state["turn_rate_rad_s"] = p.readUserDebugParameter(self.turn_rate_rad_s_slider)
        ...
        gait_state["gait_type"] = self.last_gait_type
